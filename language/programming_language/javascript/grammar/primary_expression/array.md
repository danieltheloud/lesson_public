...
- `concat`
  - `concat(x)`의 꼴에서 배열 `x`를 원본 배열의 마지막에 추가하여 반환한다.
...
- `every`
  - `every(x)`의 꼴에서 모든 요소가 메서드 `x`의 인자로써 참(true)을 반환하게 하는지 여부를 반환한다.
...
- `filter`
  - `filter(x)`의 꼴에서 메서드 `x`로 하여금 참을 반환하게 하는 모든 요소들을 얕은 복사하여 배열로 반환한다.
    - 원본 배열이 수정되는 것이 아님에 유의한다.
...
- `find`
  - `find(x)`의 꼴에서 메서드 `x`의 인자로써 참을 반환하게 하는 첫 요소를 반환한다.
...
- `findIndex`
  - `findIndex(x)`의 꼴에서 메서드 `x`의 인자로써 참을 반환하게 하는 첫 요소의 인덱스를 반환한다.
...
- `forEach`
  - `forEach(x)`의 꼴에서 각 요소를 메서드 `x`의 인자로 넘겨준다.
...
- `map`
  - `map(x)`의 꼴에서 각 요소를 메서드 `x`의 인자로써 넘겨준 결과들을 배열로 반환한다.
...
- `reduce`
  - `reduce(x)`의 꼴에서 2개의 인자를 받는 메서드 `x`를 반복적으로 호출한 결과를 반환한다.
...
- `reverse`
  - `reverse()`의 꼴에서 요소의 순서를 반전하여 반환한다.
    - 원본 배열을 수정하는 것임에 유의한다.
...
- `some`
  - `some(x)`의 꼴에서 하나 이상의 요소가 메서드 `x`의 인자로써 참을 반환하게 하는지 여부를 반환한다.
...
- `sort`
  - `sort()`의 꼴에서 모든 요소를 `UTF-16`의 기준에서 오름차순으로 정렬하여 반환한다.
    - 원본 배열을 수정하는 것임에 유의한다.
...
- `toString`
  - `toString()`의 꼴에서 모든 요소들을 문자로써 연결하여 반환한다.
...
- `with`
  - `with(x, y)`의 꼴에서 인덱스 `x`의 요소를 `y`로 교체하여 반환한다.
    - 원본 배열을 수정하는 것이 아님에 유의한다.
