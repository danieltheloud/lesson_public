...
- `at`
  - `at(x)`의 꼴에서 `x`를 인덱스로 하는 요소를 반환한다.
    ...
    ```
- `concat`
  - `concat(x)`의 꼴에서 `x`는 배열이다.
    - 원본 배열의 마지막에 `x`를 추가하여 반환한다.
    ...
    ```
- `every`
  - `every(x)`꼴에서 `x`는 논리값을 반환하는 메소드이다.
    - 모든 요소가 `x`의 인자로써 참(true)을 반환하게 하는지 여부를 반환한다.
    ```js
    ...
    ```
- `filter`
  - `filter(x)`의 꼴에서 `x`는 논리값을 반환하는 메소드이다.
    - `x`의 인자로써 참을 반환하게 하는 요소들을 얕은 복사하여 반환한다.
    - 원본 배열이 수정되는 것이 아님에 유의한다.
    ```js
    ...
    ```
- `find`
  - `find(x)`의 꼴에서 `x`는 논리값을 반환하는 메소드이다.
    - `x`의 인자로써 참을 반환하게 하는 첫 요소를 반환한다.
    ```js
    ...
    ```
- `findIndex`
  - `findIndex(x)`의 골에서 `x`는 논리값을 반환하는 메소드이다.
    - `x`의 인자로써 참을 반환하게 하는 첫 요소의 인덱스를 반환한다.
    ```js
    let myArray = [1, 2, 'three', 'four'];
    ...
    ```
- `findLast`
  - `findLast(x)`의 꼴에서 `x`는 논리값을 반환하는 메소드이다.
    - `x`의 인자로써 참을 반환하게 하는 마지막 요소를 반환한다.
      - `find` 메소드의 역순으로 순회하기 때문이다.
- `findLastIndex`
  - `findLastIndex(x)`의 꼴에서 `x`는 논리값을 반환하는 메소드이다.
    - `x`의 인자로써 참을 반환하게 하는 마지막 요소의 인덱스를 반환한다.
- `flat`
  - `flat()`의 꼴에서 하위 배열의 한 단계 떨어뜨려서 반환한다.
    ...
    ```
  - `flat(x)`의 꼴에서 `x`는 정수이다.
    - `x`번 하위 배열을 원본 배열의 요소에 포함시킨다.
    - `x`가 1 이상일 때, 의미가 있다.
...
- `includes`
  - `includes(x)`의 꼴에서 원본 배열에 `x`가 포함되어 있는지 여부를 반환한다.
    ...
    ```
- `join`
  - `join()`의 꼴에서 각 요소의 사이를 `,`로 연결하여 문자열을 반환한다.
    ```js
    let myArray = [1, 3.14, 'hi'];

    console.log(myArray.join());    // 1,3.14,hi
    ```
  - `join(x)`의 꼴에서 `x`는 문자열이다.
    - 각 요소의 사이에는 `x`가 삽입된다.
    ```js
    let myArray = [1, 3.14, 'hi'];
    ...
    ```
- `map`
  - `map(x)`의 꼴에서 `x`는 메소드이다.
    - 각 요소를 `x`의 인자로써 넘겨준 결과를 배열로 반환한다.
    ```js
    let myArray = [1, 2, 3];
